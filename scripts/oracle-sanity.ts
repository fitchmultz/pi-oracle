...
async function testOraclePromptTemplateCutover(): Promise<void> {
  ...
  assert(runtimeSource.includes("spawnRobocopy"), "runtime profile cloning should use timeout-bounded robocopy on Windows instead of unbounded fs.cp");
  assert(runtimeSource.includes("taskkill"), "runtime timeout cleanup should terminate Windows child process trees when shell wrappers are used");
  ...
  assert(configSource.includes("pathEqualsOrIsInside(runtimeProfilesDir, authSeedProfileDir)"), "config validation should reject nested runtime profile dirs with platform-aware separators");
  assert(configSource.includes("parse(pathValue).root"), "config validation should reject platform root profile directories, including Windows drive roots");
...
async function testResponseTimeoutGuard(): Promise<void> {
  ...
  assert(workerSource.includes("spawnRobocopy"), "worker runtime profile cloning should use timeout-bounded robocopy on Windows instead of unbounded fs.cp");
  ...
  assert(authBootstrapSource.includes("taskkill"), "auth bootstrap timeout cleanup should terminate Windows child process trees when shell wrappers are used");
