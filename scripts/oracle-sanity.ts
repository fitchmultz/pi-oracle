...
async function testOraclePromptTemplateCutover(): Promise<void> {
  ...
  assert(pkg.devDependencies?.["@earendil-works/pi-coding-agent"] === "^0.80.7", "package.json should use the current Pi 0.80.7 local development baseline");
  assert(pkg.devDependencies?.["@earendil-works/pi-ai"] === "^0.80.7", "package.json should use the current pi-ai 0.80.7 local development baseline");
  ...
  assert(readmeSource.includes("Pi `0.80.7+` is the suggested tested floor") && readmeSource.includes("optional wildcard peers"), "README should document the suggested Pi 0.80.7 floor without making it a hard peer requirement");
  assert(designSource.includes("pi` 0.80.7+") || designSource.includes("`pi` 0.80.7+"), "design doc should name the current suggested Pi 0.80.7 compatibility floor");
